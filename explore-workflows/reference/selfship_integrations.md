...
|---|---|---|---|---|
| 1 | LangChain & LangGraph (Python) | `selfship.langchain_callbacks()` (wraps `langfuse.langchain.CallbackHandler`) | ✅ Full — verified | Prefer the SDK helper over constructing `CallbackHandler` directly (`docs/python/integrations.mdx`, `docs/otel/langchain.mdx`). Invoke inside `workflow()`. Cross-boundary chains with no LangChain parent take `langchain_callbacks(trace_context=wf.trace_context())`. Nested invokes inside a running node must reuse `config["callbacks"]` (the live `CallbackManager`) — a fresh handler list, even with `trace_context=`, stamps `is_langchain_root` and elevates that run's I/O onto deprecated `trace.input` / `trace.output`. SelfShip still scores remapped `trace.input` as the user query. |
